    ...
    $.ajax({
        ...
        if(result!="ImE6MDp7fSI=") {
            ...
            for (var j = 0; j < convertedReturn.length; j++) {
                ...
                var query = 'SELECT * FROM schedules_solicitation WHERE user_id = ' + $.cookieStorage.get('User').id+' and voucher_id = ' +id+ ' and status = "WAITING_COMPANY_RESPONSE";';
                ...
                var query = 'SELECT * FROM services_vouchers Vouchers  INNER JOIN offers Offer ON Vouchers.offer_id = Offer.id WHERE user_id = ' + $.cookieStorage.get('User').id+' and Vouchers.id = ' +id+ ' and DATE(Offer.ends_at) < DATE(NOW());';
                ...
                $.ajax({
                    ...
                    if(result == "ImE6MDp7fSI="){
                        ...
                        for(var j=0;j<convertedReturn.length;j++){
                            $("#"+convertedReturn[j].Vouchers.id).css('opacity', '0.3');

                            document.getElementById("img"+convertedReturn[j].Vouchers.id).setAttribute('onclick', '');
                            document.getElementById(convertedReturn[j].Vouchers.id).setAttribute('onclick', '');
