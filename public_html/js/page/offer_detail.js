...
function sendRequest() {
    ...
    $.ajax({
        ...
        var date = new Date(convertedReturn.Checkout.date);
        $("#unitCheckoutDate").append(moment().format('D/MM/YYYY') );
        ...
        if(convertedReturn.Checkout.installment != 1){
            ...
            $("#unitParcel").append("1x");
        }

        $("#unitCode").append(convertedReturn.Checkout.id);
        ...
        switch(date.getMonth()){
            case 0: datemonth = 'Janeiro';
                break;
            case 1: datemonth = 'Fevereiro';
                break;
            case 2: datemonth = 'Março';
                break;
            case 3: datemonth = 'Abril';
                break;
            case 4: datemonth = 'Maio';
                break;
            case 5: datemonth = 'Junho';
                break;
            case 6: datemonth = 'Julho';
                break;
            case 7: datemonth = 'Agosto';
                break;
            case 8: datemonth = 'Setembro';
                break;
            case 9: datemonth = 'Outubro';
                break;
            case 10: datemonth = 'Novembro';
                break;
            case 11: datemonth = 'Dezembro';
                break;
        }

        if((convertedReturn.Checkout.delivery_time/1)>=1) {
            date.setTime(date.getTime()+(convertedReturn.Checkout.delivery_time/1)*24*60*60*1000);
            $("#unitDeliveryDays").append((date.getDate()) + '/' + ((date.getMonth()/1)+1) +  '/' + date.getFullYear()+'*');
        }
        else {
        ...
        $.ajax({
            ...
            var urlOffer = 'https://'+api+'/users/get/query/' + createToken();
            ...
            $.ajax({
                ...
                var convertedReturnOffers = (JSON.parse(decodeObjReturnOffers));

                var desconto = (convertedReturnOffers[0].Offer.percentage_discount)/1;

                    $.cookieStorage.set(convertedReturnOffers[0]);

                    if(Math.floor(desconto) != 0){
                        $("#offerOff").append(Math.floor(desconto)+ "%<br>OFF");
                    }else{
                        $("#offerOff").addClass('hide');
                        $("#commentsssss").css('margin-top', '18%');
                    }
                ...
                $.ajax({
                    ...
                    */
                    var conditionsPaymentMethod = {
                        'User': {
                                'query':'SELECT * FROM payment_methods PaymentMethod WHERE id = '+PaymentMethod
                        }
                    };
                    ...
                    var urlPaymentMethod = 'https://'+api+'/users/get/query/' + createToken();
                    ...
                    $.ajax({
                       ...
                       //console.log(convertedReturnPaymentMethod);
                        $("#unitPaymentForm").append(convertedReturnPaymentMethod[0].PaymentMethod.name);
