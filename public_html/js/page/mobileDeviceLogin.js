...
function sendRequest() {
    ...
    $.ajax({
        ...
        if (result != "") {
			...
			console.log("DECODE RETURN:" + decodeObjReturn);			
			
            var convertedReturn = JSON.parse(decodeObjReturn);

            console.log("CONVERTED RETURN:" + convertedReturn);
			...
			//capturamos o REG ID do usuário para atualizar no banco de dados
            if ($.cookieStorage.isSet('User')) {
			 
				var regID = getUrlVars()['regid'];
				
				var sqlUpdate = "update users_using set reg_id = '"+regID+"' where user_id = "+id;
                ...
                var postData = JSON.stringify(conditions1);
...
function generateModalAlert(mensagem) {
    if ($("#mymodal").length) {
        ...
        $modalHtml =
            '<div class="modal bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" id="mymodal">'
