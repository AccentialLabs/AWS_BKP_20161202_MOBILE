...
$(document).ready(function () {
    ...
    var query = "SELECT * FROM notifications_company INNER JOIN companies ON notifications_company.company_id INNER JOIN secondary_users ON notifications_company.secondary_user_id = secondary_users.id WHERE companies.id = notifications_company.company_id and notifications_company.user_id =" +$.cookieStorage.get('User').id + " and notifications_company.status = 1 and notifications_company.data >= DATE_FORMAT(CURRENT_TIMESTAMP, '%Y-%m-%d') and notifications_company.date_notification >= DATE_FORMAT(CURRENT_TIMESTAMP - interval 7 day, '%Y-%m-%d') or companies.id = notifications_company.company_id and notifications_company.user_id =" +$.cookieStorage.get('User').id + " and notifications_company.status = 4 and notifications_company.description != 'NOVA OFERTA ADICIONADA' and notifications_company.data >= DATE_FORMAT(CURRENT_TIMESTAMP, '%Y-%m-%d') and notifications_company.date_notification >= DATE_FORMAT(CURRENT_TIMESTAMP - interval 7 day, '%Y-%m-%d') or companies.id = notifications_company.company_id and notifications_company.user_id =" +$.cookieStorage.get('User').id + " and notifications_company.status = 1 and notifications_company.data = '00:00:00' and notifications_company.date_notification >= DATE_FORMAT(CURRENT_TIMESTAMP - interval 7 day, '%Y-%m-%d') or companies.id = notifications_company.company_id and notifications_company.user_id =" +$.cookieStorage.get('User').id + " and notifications_company.status = 4 and notifications_company.description != 'NOVA OFERTA ADICIONADA' and notifications_company.data = '00:00:00' and notifications_company.date_notification >= DATE_FORMAT(CURRENT_TIMESTAMP - interval 7 day, '%Y-%m-%d') ORDER BY notifications_company.peso  DESC, notifications_company.data DESC;";
            ...
            if (notifications > 10) {
                for (var n = 0; n < 10; n++) {
                    ...
                    if (notification_title == 'SUGESTAO DE NOVO HORARIO') {
						...
						if(notification_title.indexOf('FELIZ')!=-1){
							 HTML ="<div class='service_class tablecontenttitle "+classe+"' onclick='Click("+objReturn1+",\""+notification_title+"\")'><img class='icones' style='width:12%;margin-top:-2%;' src="+icone+"> "+notification_title+"<img class='icones2' src='img/icons/lixeirabranca.png' onclick='ApagarNotificacao("+classe1.notifications_company.id+");'></div><table class='table' width=100%> <tr><td align='center'>"+classe1.secondary_users.name+"</td><td align='center'></td><td align='right'>"+classe1.companies.corporate_name+"</td></tr><tr><td></td><td></td><td align='right' class='tdnotification'>"+datanotificationformatada+"</td></tr></table> <div class='bottomLine3'></div> ";
						}else{
                ...
                for(var n = 0; n<notifications;n++){
                    ...
                    if(notification_title == 'SUGESTAO DE NOVO HORARIO'){
                        ...
                        var datanotification =  data;
                        var datanotificationformatada = moment(datanotification).format('DD/MM/YYYY');
                    }else if(notification_title.indexOf('FELIZ')!=-1){
                        var negrito = '<span style="font-weight:600">ANIVERSÁRIO</span>';
                        notification_title = $.cookieStorage.get('User').name.split(' ')[0] +', FELIZ '+negrito+'';
                        icone ='img/icons/aniversariante.png';
                        classe = 'tablecontentcancel';                        
                        horario = classe1.secondary_users.name;
                        data = classe1.notifications_company.date_notification;
                        var datanotification =  data;
                        var datanotificationformatada = moment(datanotification).format('DD/MM/YYYY');
                    }else if(notification_title == 'NOVA OFERTA ADICIONADA'){
						...
						if(notification_title.indexOf('FELIZ')!=-1){
							 HTML ="<div class='service_class tablecontenttitle "+classe+"' onclick='Click("+objReturn1+",\""+notification_title+"\")'><img class='icones' style='width:12%;margin-top:-2%;' src="+icone+"> "+notification_title+"<img class='icones2' src='img/icons/lixeirabranca.png' onclick=\'ApagarNotificacao("+classe1.notifications_company.id+");\'></div><table class='table' width=100%> <tr><td align='center'>"+horario+"</td><td align='center'></td><td align='right'>"+classe1.companies.corporate_name+"</td></tr><tr><td></td><td></td><td align='right' class='tdnotification'>"+datanotificationformatada+"</td></tr></table> <div class='bottomLine3'></div>";
						}else{
...
function volta(){
	...
	if(window.referrer != 'https://secure.jezzy.com.br/public_html/home.php'){
		window.history.go(-1);
	}else{
