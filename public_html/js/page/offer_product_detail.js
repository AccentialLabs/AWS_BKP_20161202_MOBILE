...
$(document).ready(function() {
    ...
    if(document.referrer == "https://"+ip+"/jezzy-mobile/public_html/home.html"){
    ...
    $("#buyButton").click(function(){
        var conditions = {
            'User': {
                'query': 'SELECT id, resume, SKU, amount_allowed, begins_at, brand, classification, company_id, cost, ends_at, line, metrics, parcels, parcels_off_impost, parcels_quantity, percentage_discount, percentage_discount_2, percentage_discount_3, photo, public, purchase_price, status, title, value, value_2, value_3, weight FROM offers Offer WHERE id = ' + id
            }
        };
...
function sendRequest() {
    var conditions = {
        'User': {
            'query': 'SELECT * FROM offers Offer INNER JOIN companies Company ON Offer.company_id = Company.id WHERE Offer.id = ' + id
        }
    };
   ...
   //console.log(conditions);
    ...
    $.ajax({
       ...
       //console.log(result);
        ...
        var convertedReturn = JSON.parse(decodeObjReturn);
            convertedReturn = convertedReturn[0];
            if(convertedReturn.Offer.company_id != 99999){
                $("#buyButton").html('<div class="col-xs-12" style="line-height: 1;font-size:4.8vw;"><span>COMPRAR DO SALÃO</span></div><div class="col-xs-12" style="line-height: 1;"><small>'+convertedReturn.Company.fancy_name.toUpperCase()+'</small></div>');
                $("#buyButton").css('padding', '2%');
                $("#carousel-example-generic").prepend('<img src="img/icons/seta_oferta_salao.png" style="top:0%;float:left;width: 8%;position: absolute;z-index:99999;">');
            }
            ...
            if(convertedReturn.Offer.photo == ""){
                ...
                $("#left").addClass("hide");
                $("#right").addClass("hide");
                $("#offerimages").append( '<div style="z-index: -1;" class="item active"><img src=\"'+icons+'ImagemIndisponivel2.png\" class="offerPhoto img-responsive center-block" id="offerPhoto"/></div>')
            }else{
                ...
                $.ajax({
                    ...
                    if(result!="ImE6MDp7fSI="){
                        ...
                        $("#offerimages").html("");
                        $("#indicadores").html("");
                        $("#offerimages").append('<div style="z-index: -1;" class="item active"><img class="offerPhoto img-responsive center-block" src="'+ convertedReturn.Offer.photo+'" /></div>');
                        ...
                        for(var h=0;h<convertedReturnss.length;h++){
                            $("#offerimages").append('<div style="z-index: -1;" class="item"><img class="offerPhoto img-responsive center-block" src="'+ convertedReturnss[h].offers_photos.photo+'" /></div>');
		...
		if(Math.round(convertedReturn.Offer.percentage_discount) != 0){
			  $("#circle").html(Math.round(convertedReturn.Offer.percentage_discount)+'% OFF');
		}else{
			$("#circle").hide();
		}
        ...
        $("#footerHomeIconClick").click(function(){
            if(convertedReturn.Offer.description != null && convertedReturn.Offer.description != ''){
                generateModalAlert(convertedReturn.Offer.description);
                $('#mymodal').modal('show');
            }else{
                generateModalAlert('Sem informações');
                $('#mymodal').modal('show');
            }
        });
            $("#resultsFooterIconClick").click(function(){
                if(convertedReturn.Offer.result != null && convertedReturn.Offer.result != ''){
                    generateModalAlert(convertedReturn.Offer.result);
                    $('#mymodal').modal('show');
                }else{
                    generateModalAlert('Sem informações');
                    $('#mymodal').modal('show');
            ...
            $("#footerEspecificacaoClick").click(function(){
                if(convertedReturn.Offer.specification != null && convertedReturn.Offer.specification != ''){
        ...
        $.ajax({
            ...
            switch(media){
                ...
                $("#coments").append('<span style="color: grey;font-size: 1.5vh;">'+convertedReturnComment.length+' COMENTÁRIOS</span>');

            }else{
                $("#numberStars").attr('src', "img/icons/0.png");
                $("#coments").append('<span style="color: grey;font-size: 1.5vh;">NÃO HÁ COMENTÁRIOS</span>');
            }
